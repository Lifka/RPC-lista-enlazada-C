...
{
	return (ponerasociacion_1_svc(argp->id, argp->clave, argp->valor, rqstp));
}

static valor_estado *
_obtenerasociacion_1 (obtenerasociacion_1_argument *argp, struct svc_req *rqstp)
{
	return (obtenerasociacion_1_svc(argp->id, argp->clave, rqstp));
}

static estado *
_borrarasociacion_1 (borrarasociacion_1_argument *argp, struct svc_req *rqstp)
{
	return (borrarasociacion_1_svc(argp->id, argp->clave, rqstp));
}
...
asociacion_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	...
	case enumerar:
		...
		_xdr_result = (xdrproc_t) xdr_enumerados;
...
main (int argc, char **argv)
{
	...
	pmap_unset (ASOCIACION, ASOCIACION_1);
	...
	if (!svc_register(transp, ASOCIACION, ASOCIACION_1, asociacion_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (ASOCIACION, ASOCIACION_1, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, ASOCIACION, ASOCIACION_1, asociacion_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (ASOCIACION, ASOCIACION_1, tcp).");
		exit(1);
	}
